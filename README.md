```markdown name=SKILL.md url=https://github.com/doudouma/uploadhtmlskill/blob/main/SKILL.md
---
name: 100mini-upload
...
- Points policy: same as the web UI — free permanent-page quota (default 5 + bonus) applies; beyond that each upload costs 10 points. Active members are exempt from points.
...
| 特性 | 匿名模式 | 认证模式 |
|------|--------|--------|
| **API Token** | 不需要 | 必需 (`100m_...`) |
| **每日限制** | 5次/IP/天 | 无限制 |
| **页面保留** | 7天自动删除 | 永久保存 |
| **费用** | 免费 | 前5页免费，超出10积分/页 |
| **必要参数** | `--file`/`--content` | `--file` + `--title` |

**最常见用途：** 用户说"上传到100mini"或"生成分享链接" → 直接用此Skill上传HTML/ZIP，返回分享链接
